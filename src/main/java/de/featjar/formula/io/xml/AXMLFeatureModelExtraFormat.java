 ...
 * @author Elias Kuiter
 */
public abstract class AXMLFeatureModelExtraFormat<T, U, V> extends AXMLFormat<T> {
    ...
    protected abstract U newFeatureLabel(
            String name, U parentFeatureLabel, boolean mandatory, boolean _abstract, boolean hidden, boolean or, boolean alt)
    ...
    protected void parseFeatureTree(Element element) throws ParseException {
        parseFeatureTree(element.getChildNodes(), null, false,false,false);
    }

    protected ArrayList<U> parseFeatureTree(NodeList nodeList, U parentFeatureLabel, boolean and, boolean or, boolean alt)
            throws ParseException {
        ...
        for (final Element e : elements) {
            ...
            switch (nodeName) {
                ...
                case ALT:
                case FEATURE:
                    featureLabels.add(parseFeature(parentFeatureLabel, e, nodeName, and,or,alt));
                    break;
                default:
                    break;
            }
    ...
    protected U parseFeature(U parentFeatureLabel, final Element e, final String nodeName, boolean and, boolean or, boolean alt )
            throws ParseException {
        ...
        U featureLabel = newFeatureLabel(name, parentFeatureLabel, and && mandatory, _abstract, hidden, alt, or);

        if (e.hasChildNodes()) {
            final ArrayList<U> featureLabels = parseFeatureTree(e.getChildNodes(), featureLabel, nodeName.equals(AND), nodeName.equals(OR),nodeName.equals(ALT));
            switch (nodeName) {
                case AND:
